...
export default function AddAddressDetails(props) {
    ...
    const handleClick = async () => {
        if (town && pincode && city && state && address) {
            var result = await postData('userinterface/submit_useraddress', { userid: mobileNumber, pincode: pincode, town: town, city: city, state: state, address: address })
            if (result.status) {
    ...
    const handleChangeAddress = async () => {
        if (town && pincode && city && state && address) {
            var result = await postData('userinterface/update_useraddress', { userid: mobileNumber, pincode: pincode, town: town, city: city, state: state, address: address })
            if (result.status) {
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                {changeAddress ?
                    <div>
                        <Button onClick={handleChangeAddress} variant="contained" style={{ background: 'rgb(81, 203, 204)', color: 'white' }}>Change Address</Button>
                    </div>
                    :
                    <div>
                        <Button onClick={handleClick} variant="contained" style={{ background: 'rgb(81, 203, 204)', color: 'white' }}>Add Address</Button>
                    </div>
                }
            </Grid>
